...
void read_ultrasonic(int trigPin, int echoPin, int& distance) {
...
void codeForTask1(void* parameter) {
  for (;;) {
    read_ultrasonic(trig_f, echo_f, distance_f);
    read_ultrasonic(trig_r, echo_r, distance_r);
    read_ultrasonic(trig_l, echo_l, distance_l);
...
void loop() {
  ...
  if (distance_f < head_range || distance_r < head_range || distance_l < head_range) {  // (Y.O.L.O.)ing
    ...
    attack();
  } else if (distance_r < range && distance_l > range && ir_fr == 1) {  // detect the opponent on the right front
    forwardright(full);
  } else if (distance_r > range && distance_l < range && ir_fl == 1) {  // detect the opponent on the left front
    forwardleft(full);
